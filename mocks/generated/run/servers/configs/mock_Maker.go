...
func (_m *MockMaker) Make(config map[string]types.ServerConfig) (configs.Configs, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(map[string]types.ServerConfig) (configs.Configs, error)); ok {
		return rf(config)
	}
	if rf, ok := ret.Get(0).(func(map[string]types.ServerConfig) configs.Configs); ok {
		r0 = rf(config)
	} else {
		if ret.Get(0) != nil {
	...
	if rf, ok := ret.Get(1).(func(map[string]types.ServerConfig) error); ok {
		r1 = rf(config)
	} else {
		r1 = ret.Error(1)
	}
...
func (_c *MockMaker_Make_Call) Run(run func(config map[string]types.ServerConfig)) *MockMaker_Make_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(map[string]types.ServerConfig))
	})
	return _c
}
