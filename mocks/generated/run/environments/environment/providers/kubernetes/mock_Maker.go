...
func (_m *MockMaker) Make(config *types.KubernetesEnvironment) (environment.Environment, error) {
	...
	var r1 error
	if rf, ok := ret.Get(0).(func(*types.KubernetesEnvironment) (environment.Environment, error)); ok {
		return rf(config)
	}
	if rf, ok := ret.Get(0).(func(*types.KubernetesEnvironment) environment.Environment); ok {
		r0 = rf(config)
	} else {
		if ret.Get(0) != nil {
