...
func TestActionMaker_Make(t *testing.T) {
	tests := []struct {
		...
		{
			name: "successful restart action creation with default timeout",
			config: &types.RestartAction{
		...
		{
			name: "successful restart action creation with set timeout",
			config: &types.RestartAction{
		...
		{
			name:           "successful restart action creation without any service",
			config:         &types.RestartAction{},
			defaultTimeout: 5000,
			setupMocks: func(t *testing.T, sl *servicesMocks.MockServiceLocator) services.Services {
		...
		{
			name: "failed restart action creation when service not found",
			config: &types.RestartAction{
	...
	tests := []struct {
	...
	}{
		{
			...
			setupMocks: func(t *testing.T, ctx context.Context) services.Services {
				...
				svc2.On("Name").Return("s2")
				svc3.On("Name").Return("s3")
				svc1.On("Restart", ctx).Return(nil)
				svc2.On("Restart", ctx).Return(nil)
				svc3.On("Restart", ctx).Return(nil)
				return services.Services{
					"s1": svc1,
					"s2": svc2,
					"s3": svc3,
		...
		{
			name: "failed execution of restart",
			setupMocks: func(t *testing.T, ctx context.Context) services.Services {
				svc1 := servicesMocks.NewMockService(t)
				svc2 := servicesMocks.NewMockService(t)
				svc3 := servicesMocks.NewMockService(t)
				...
				svc1.On("Restart", ctx).Maybe().Return(nil)
				svc2.On("Restart", ctx).Return(errors.New("e2 failure"))
				svc3.On("Restart", ctx).Maybe().Return(nil)
				return services.Services{
					"s1": svc1,
					"s2": svc2,
					"s3": svc3,
